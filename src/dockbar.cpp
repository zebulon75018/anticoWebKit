...
Dockbar::Dockbar(Antico *a, QWidget *parent) : QLabel(parent)
{
    ...
    connect(home, SIGNAL (released()), this, SLOT (gotoHome()));

    QPushButton* keyboard = new	QPushButton("Keyboard");
    connect(keyboard, SIGNAL (released()), this, SLOT (showKeyBoard()));
    ...
    dock_layout->insertWidget(6, keyboard);
    dock_layout->insertWidget(7, home);
...
void Dockbar::read_settings()
{
    ...
    // get style values
	qDebug() << "style " << stl_path + stl_name;
...
void Dockbar::set_geometry()
{
    ...
    int space_dock = (QApplication::desktop()->width()-dock_width)/2; // space left on right/left side of Dockbar
       ...
       qDebug() << " setGeometry " << space_dock <<" " << QApplication::desktop()->height()-dock_height <<" " << dock_width << " " <<  dock_height;
...
void Dockbar::gotoHome() {
	app->get_desktop()->getWebView()->setUrl(QUrl("file:///home/pi/html/pages/index.html"));

}
void Dockbar::showKeyBoard() {
  //QWidget *k = new QWidget(app->get_desktop());
  //QLabel* lbl = new QLabel("keyboard window...", app->get_desktop());
  //app->get_desktop()->getLayout()->addWidget(lbl);
  app->get_desktop()->toggleKeyBoard();
  //lbl->show();

  //k->setWindowFlags(Qt::WindowStaysOnTopHint);
  //k->move( this->pos() );
  //k->resize( 200,200 );
  //k->show();
}
